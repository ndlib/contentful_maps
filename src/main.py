...
missingQueryParams = {
  "statusCode": 422,
  "body": "Missing Query Parameters: expected call_number, collection, sublibrary.",
}
...
def handler(event, context):
    ...
    matches = filter(lambda x: (
        'rangeStart' not in x or not x['rangeStart'] or x['rangeStart'] <= call_number
    ) and (
        'rangeEnd' not in x or not x['rangeEnd'] or x['rangeEnd'] >= call_number
    ), callmap)
